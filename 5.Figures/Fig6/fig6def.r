...
source("customize_Seurat_FeaturePlot.r")
...
pdf(paste0("14.TSNEPlot_Ann", KeyS, ".pdf"), width = 11)
	...
	DimPlot(Obj, reduction = "umap", label = TRUE, repel = FALSE, pt.size = 0.3, label.size = 6, group.by = "category_anno_meta", cols = Colors[1:length(unique(category_anno[[3]]))]) + theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title.x = element_text(size = 18), axis.title.y = element_text(size = 18), legend.position = "none")
...
if(OrderS != "Null"){
	...
	Idents(Obj) <- factor(as.character(Obj$category_anno_meta), levels = rev(unlist(strsplit(OrderS, split = ";"))))
...
markers_cc <- factor(rev(markers_cc), levels = (markers_cc))

pdf(paste0("13.Dot_plot_", KeyS, ".pdf"), width = 0.3 *length(markers_cc) + 5, height = 0.5 * length(unique(Obj$category_anno_meta)))

	DotPlot(Obj, assay = "RNA", features = markers_cc, cols = brewer.pal(9, "YlOrRd")[c(3, 9)], dot.scale = 8, scale = TRUE) + RotatedAxis() + theme(axis.text.x = element_text(size=13), axis.text.y = element_text(size = 13), axis.title.y = element_blank(), axis.title.x = element_blank())
